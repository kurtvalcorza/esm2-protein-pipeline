...
from esm2_protein_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
    ESM2Pipeline,
...
_CONFIG = json.dumps({"model_type": "esm", "hidden_size": 640}).encode()
...
def test_from_pretrained_refuses_without_manifest_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="no snapshot manifest"):
        ESM2Pipeline.from_pretrained(device="cpu", weights_dir=tmp_path, allow_download=False)
...
def test_invalid_sequences_are_rejected_before_model_imports(forbid_model_imports):
