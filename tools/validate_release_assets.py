...
REPO_NAME = "esm2-protein-pipeline"
NOTEBOOK_NAME = "esm2_protein_colab.ipynb"
EXPECTED_PROFILE = "E2E"
EXPECTED_MODEL_ID = "facebook/esm2_t30_150M_UR50D"
PIPELINE_CLASS = "ESM2Pipeline"
# INF1: the load expression the model cell must carry (one pinned snapshot, the default form).
MODEL_LOAD_EXPR = f"{PIPELINE_CLASS}.from_pretrained(weights_dir=WEIGHTS_DIR)"
# Additional 40-hex revisions a document may legitimately cite: none; this pipeline pins one snapshot.
KNOWN_SHAS: frozenset[str] = frozenset()
# Colab form gates that must default to the non-interactive sample path.
BYOD_GATES = ("USE_BYOD",)
# Machine-readable artifacts the notebook must write (OUT1-OUT4, OUT8).
EXPECTED_OUTPUTS = (
    "outputs/esm2_protein_sample_dataset.csv",
    "outputs/esm2_protein_embeddings.csv",
    "outputs/esm2_protein_evaluation_report.json",
    "outputs/esm2_protein_predictions.csv",
    "outputs/esm2_protein_adapter",
    "outputs/esm2_protein_result.json",
)
# Profile-specific code the notebook must exercise through the carried module's public API.
CODE_MARKERS = (
    # Stage 4: dataset, validation, stratified split
    "USE_BYOD = False",
    ...
    # Stage 8: held-out evaluation and report
    ...
    # Stage 9: new-data inference through validate_inputs + classify
    ...
    # Stage 10: artifact export and fresh reload parity
...
EXTERNAL_WEIGHT_BYTES: dict[int, str] = {
    593_355_136: "upstream tf_model.h5 at facebook/esm2_t30_150M_UR50D@a695f60 (not staged)",
    595_364_077: "upstream pytorch_model.bin at facebook/esm2_t30_150M_UR50D@a695f60 (not staged)",
}
